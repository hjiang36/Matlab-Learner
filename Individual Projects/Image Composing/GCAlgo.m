...
function finalLabel = GCAlgo(im, fixedBG,  K, G, maxIter, Beta, diffThreshold)
...
for iter=1:maxIter
    ...
    fgLogPL(fixedBG>0.5) = max(fgLogPL(:));
    bgLogPL(fixedBG<0)   = max(bgLogPL(:));
    ...
    graphHandle = GraphCut('set', graphHandle, prevLabel>0.5);
